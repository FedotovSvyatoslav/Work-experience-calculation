...
from src.core import exp_calculate as exp_c
from src.gui import dialog_wnds
from src.db import db_handlings as db_h
from src.core.my_exceptions.my_exceptions import OpenException


CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
RESOURCES_DIR = os.path.join(CURRENT_DIR, "resources")

MAINWINDOW_UI = os.path.join(RESOURCES_DIR, "MainWindow5.ui")
WINDOW_ICON_FILE = os.path.join(RESOURCES_DIR, "staj_icon.ico")
CALENDAR_ICON_FILE = os.path.join(RESOURCES_DIR, "calendar_icon.png")
PLUS_ICON_FILE = os.path.join(RESOURCES_DIR, "plus_icon.png")
MINUS_ICON_FILE = os.path.join(RESOURCES_DIR, "minus.png")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.search_lineedit.textChanged.connect(lambda: self.update_table('searching'))
    ...
    def fill_menubar(self):
        ...
        save_to_txt_action = QAction(self)
        save_to_txt_action.setText('Save')
        save_to_txt_action.triggered.connect(self.save_result)

        file_menu.addActions([open_action, new_action, save_to_txt_action])
    ...
    def save_result(self):
        """
        Сохраняет данные о стаже выбранного работника в текстовый файл.
        """
        selected_rows = set()
        ...
        if not selected_rows:
            QMessageBox.warning(self, "Ошибка",
                                "Выберите работника для сохранения.")
            return

        if len(selected_rows) > 1:
            QMessageBox.warning(self, "Ошибка",
                                "Выберите только одного работника.")
            return

        row = list(selected_rows)[0]
        worker_id = self.Table.item(row, 0).text()
        if not worker_id:
            QMessageBox.warning(self, "Ошибка", "Выбран пустой работник.")
            return

        worker_fio = self.Table.item(row, 1).text()
        profession = self.Table.item(row, 2).text()
        entered_date = self.Table.item(row, 3).text()
        total_exp_before = self.Table.item(row, 4).text()
        pedagogic_exp_before = self.Table.item(row, 5).text()
        total_exp = self.Table.item(row, 6).text()
        pedagogic_exp = self.Table.item(row, 7).text()

        result = (
            f"Работник: {worker_fio} (ID: {worker_id})\n"
            f"Текущая профессия: {profession}\n"
            f"Дата вступления: {entered_date}\n"
            f"Общий стаж до вступления: {total_exp_before}\n"
            f"Педагогический стаж до вступления: {pedagogic_exp_before}\n"
            f"Общий стаж: {total_exp}\n"
            f"Педагогический стаж: {pedagogic_exp}\n"
        )

        try:
            with open(f"worker_{worker_id}_experience.txt", "w",
                      encoding="utf-8") as f:
                f.write(result)
            QMessageBox.information(
                self, "Успех", f"Результат сохранен в worker_{worker_id}_experience.txt")
